...
public class SBT_SpriteSheet
{
    ...
    static List<SpriteMetaData> ParseSprites(XElement xml, int texH)
    {
        ...
        foreach (XElement e in elements)
        {
            ...
            r.y = texH - (float)e.Attribute("y") - r.height;
            ...
            sprite.pivot = new Vector2(
                (float)((float)e.Attribute("fw") / 2 - (float)e.Attribute("fx")) / r.width,
                (r.height - ((float)e.Attribute("fh") / 2 - (float)e.Attribute("fy"))) / r.height);
